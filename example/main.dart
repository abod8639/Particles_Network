...
class _ParticleControllerScreenState extends State<ParticleControllerScreen> {
    ...
    return Container(
      ...
      child: ListView(
        children: [
          ...
          _buildSlider("Speed *", maxSpeed, 0.1, 5.0, (v) {
            setState(() => maxSpeed = v);
            _refreshEngine();
          }),
          _buildSlider("Max Size *", maxSize, 1.0, 15.0, (v) {
            setState(() => maxSize = v);
            _refreshEngine();
          }),
        ],
  ...
  Widget _buildColorSection() {
    return Row(
      ...
      children: [
        _buildColorButton("Particle", particleColor, (c) => setState(() => particleColor = c)),
        _buildColorButton("Line", lineColor, (c) => setState(() => lineColor = c)),
        _buildColorButton("Touch", touchColor, (c) => setState(() => touchColor = c)),
      ],
    );
  }

  // Individual color selection button 
  Widget _buildColorButton(String label, Color currentColor, Function(Color) onSelect) {
    return Column(
      children: [
  ...
  void _showColorPicker(String label, Function(Color) onSelect) {
    List<Color> palette = [
      Colors.white, Colors.redAccent, Colors.greenAccent, Colors.blueAccent,
      Colors.amberAccent, Colors.purple, Colors.cyanAccent, Colors.pinkAccent,
    ];
    showDialog(
      context: context,
      builder: (context) => AlertDialog(
        title: Text("Select $label Color", style: const TextStyle(fontSize: 16)),
        content: Wrap(
  ...
  Widget _buildSwitchesSection() {
    return SingleChildScrollView(
      ...
      child: Row(
        children: [
          _buildSwitch("Network", drawNetwork, (v) => setState(() => drawNetwork = v)),
          _buildSwitch("Fill", isFill, (v) => setState(() => isFill = v)),
          _buildSwitch("Complex", isComplex, (v) => setState(() => isComplex = v)),
          _buildSwitch("Touch", touchActivation, (v) => setState(() => touchActivation = v)),
        ],
      ),
    );
  }
  ...
  Widget _buildSlider(String label, double value, double min, double max, Function(double) onChanged) {
    return Row(
      children: [
        SizedBox(width: 80, child: Text(label, style: const TextStyle(fontSize: 10))),
        Expanded(
          child: Slider(
            value: value,
            min: min,
